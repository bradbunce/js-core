...
The persistent cache (anonymous-key persistence and last-known flag values)
is now configured via a single `storage` option on `LDOptions`, replacing
the previous `localStoragePath` field.

`storage` is a discriminated union: `{ type: 'file', localStoragePath? }`
selects the file-backed default; `{ type: 'custom', implementation }` lets
the application inject its own `Storage` implementation (in-memory,
encrypted-on-disk, system keychain, etc.).

Before:

```typescript
const client = createClient(envKey, initialContext, {
  localStoragePath: '/var/cache/ldclient',
});
```

After (file-backed, custom directory):

```typescript
const client = createClient(envKey, initialContext, {
  storage: { type: 'file', localStoragePath: '/var/cache/ldclient' },
});
```

After (file-backed, default directory):

```typescript
// Equivalent to omitting `storage` entirely.
const client = createClient(envKey, initialContext, {
  storage: { type: 'file' },
});
```
...
```typescript
import type { LDStorage } from '@launchdarkly/node-client-sdk';

const inMemory: LDStorage = {
  get: async (key) => cache.get(key) ?? null,
  set: async (key, value) => { cache.set(key, value); },
  clear: async (key) => { cache.delete(key); },
};

const client = createClient(envKey, initialContext, {
  storage: { type: 'custom', implementation: inMemory },
});
```

Custom implementations must not throw exceptions and should be safe for
single-SDK-instance use (multiple SDK instances sharing the same key
should not share the same persistence instance unless the implementation
is explicitly designed for it).
